        ...
        if (key == '-D' or key == '--device') and validation(rings_files):
            if len(sys.argv) > i+1:
                ...
                # HIDE MOD
                    ...
                    print('Был запущен скрипт на разворот кольца!\n'
                          'Всю дальнейшую информацию смотрите на почте либо телеграм\n'
                          '\n'
                          'Разворот НЕ будет совершен, если:\n'
                          '    - данное оборудование не входит в кольцо либо оно не записано в файле колец\n'
                          '    - все узлы доступны\n'
                          '    - кольцо уже в данный момент разворачивается (смотреть почту или телеграм)\n'
                          '    - кольцо уже развернуто (смотреть почту или телеграм) в данной ситуации, если вам '
                          'необходимо развернуть кольцо, то воспользуйтесь опцией "(MotR) Сброс кольца"\n'
                          '    - admin down не был найден на тех узлах, что доступны (находится на недоступном узле)\n'
                          '    - абсолютно все оборудование, которое входит в кольцо недоступно (агрегация также)\n'
                          '    - возникла ошибка (смотреть почту или логи)\n'
                          )
